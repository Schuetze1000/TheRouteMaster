...
function Weather() {
  ...
  const mainProcess = () => {
    ...
    Geolocation.checkPermissions().then((PermissionStatus) => {
      if (
        String(PermissionStatus.location) === "prompt" ||
        String(PermissionStatus.location) === "granted"
      ) {
        Geolocation.getCurrentPosition().then((position) => {
          ...
          axios(optionsWeatherAPI).then((resWeatherAPI) => {
            ...
            axios(optionsCity).then((resCity) => {
              ...
              setWindDirection(
                windDirectionToStr(dataWeatherAPI.current_weather.winddirection)
              );
              ...
              setApparentTemperature(
                dataWeatherAPI.hourly.apparent_temperature[timeindex]
              );
  ...
  return (
    <div className="w-56 sm:w-96 rounded-xl shadow-xl shadow-gray-50/5 bg-gray-800 bg-opacity-25 text-white mt-20 mr-auto ml-auto mb-0 pt-0 pl-5 pr-5 pb-0">
      <div className="flex justify-between items-center">
        <div>
          <p className="font-bold text-base sm:text-xl m-0 tracking-normal">
            {city}
          </p>
      ...
      <div className="flex justify-between items-center">
        <p className="font-bold text-2xl sm:text-6xl w-auto tracking-tighter">
          {temperature}°C
        </p>
        <div className="w-full p-5">
          <div className="flex justify-between">
            <span className="text-left font-bold  text-2xs sm:text-xs">
              Details
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-left font-normal text-2xs sm:text-xs">
              Gefühlt
            </span>
            <span className="text-right font-semibold text-2xs sm:text-xs">
